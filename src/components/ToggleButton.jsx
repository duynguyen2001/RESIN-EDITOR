...
const ToggleButton = ({ name, id, relatedEventsLength, chosen = false }) => {
    ...
    const [chosenEntities, setChosenEntities] = useStore((state) => [state.chosenEntities, state.setChosenEntities]);
    ...
    return (
        ...
        >
            {name.length > 40 ? `${name.slice(0, 40)}... (${relatedEventsLength})` : `${name} (${relatedEventsLength})`}
            <input
                type="checkbox"
                id={id}
                name={name}
